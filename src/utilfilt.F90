...
! -----------------------------------------------------------------
! ---
! --- FUNCTION calcfilt  : Compute element of filter convolution matrix
! --- FUNCTION filterfun : Filter attenuation function in spectral space
! --- SUBROUTINE dhankl  : Compute Hankel transform
! ---
! -----------------------------------------------------------------
!%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
      MODULE utilfilt
...
! -----------------------------------------------------------------
!%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
      FUNCTION calcfilt (kdelr)
!---------------------------------------------------------------------
!
!  Purpose : Compute element of filter convolution matrix
!  -------   (as a function of the distance with respect
!            to central point)
!
!  Method :  Select appropriate computation as a function
!  ------    of filter type (spline, constant, ...)
!
!  Input :   kdelr : distance with respect to central point
!  -----
!  Output :  calcfilt : value of filter convolution matrix
!  ------
!---------------------------------------------------------------------
! modules
! =======
...
! header declarations
! ===================
      BIGREAL, intent(in) :: kdelr
      BIGREAL :: calcfilt
!----------------------------------------------------------------------
! local declarations
! ==================
      BIGREAL :: epsilon, filtval
      INTEGER :: nb, nrel, ntol, nbrfun, ierr, nspace, allocok
      INTEGER, dimension(:), allocatable :: nord
      INTEGER, dimension(:,:), allocatable :: ijrel
      BIGREAL :: tol,r
      BIGREAL, dimension(:), allocatable :: arg
      BIGREAL, dimension(:,:), allocatable :: dwork, val
!---------------------------------------------------------------------
!
! Select appropriate computation as a function of filter type
!
...
!
         ...
         nspace = 801
!
         allocate ( nord(1:nrel), stat=allocok )
         IF (allocok.NE.0) GOTO 1001
         allocate ( ijrel(2,1:nrel), stat=allocok )
         IF (allocok.NE.0) GOTO 1001
         allocate ( arg(1:nb), stat=allocok )
         IF (allocok.NE.0) GOTO 1001
         allocate ( val(1:nb,1:nrel), stat=allocok )
         IF (allocok.NE.0) GOTO 1001
         allocate ( dwork(1:nspace,1:nrel), stat=allocok )
         IF (allocok.NE.0) GOTO 1001
!
         tol = FREAL(0.0)
         ijrel(1:2,1:nrel) = 1
         nord(1:nrel) = 0
!     
! Compute Hankel transform of 
! filter attenuation function in spectral space (filterfun)
! at distance r
!
         CALL dhankl(r,nb,nrel,tol,ntol,nord,filterfun,ijrel,dwork, &
     &               val,arg,nbrfun,ierr)
!
         filtval = val(1,1)
!
         IF (allocated(nord))  deallocate (nord)
         IF (allocated(ijrel))  deallocate (ijrel)
...
!
      CASE('constant')
!
! CONSTANT filter (running average over selected window)
!
         filtval = FREAL(1.0)
!
      CASE DEFAULT
...
! --- error management
!
 1000 CALL printerror2(0,1000,1,'utilfilt','calcfilt')
 1001 CALL printerror2(0,1001,3,'utilfilt','calcfilt')
!
 102  WRITE (texterror,*) 'incorrect filter type : ', &
     &                       filt_typ(1:lenv(filt_typ))
      CALL printerror2(0,105,3,'utilfilt','calcfilt',comment=texterror)
!
      END FUNCTION
!%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
! -----------------------------------------------------------------
!%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
      FUNCTION filterfun (kk)
!---------------------------------------------------------------------
!  
!  Purpose : Filter attenuation function in spectral space
!  -------   (corresonding to spline filter)
!
!  Method : Compute filter attenuation function value
!  ------   as a function of wave number value
!           using filter parameters in module 'mod_cont.F90'
!                                             p  2p  2p
!           Spline filter:  f(k) = 1 / sum ( a  k   l   )
!                                       p
!  Input :  kk : wave number value
!  ----- 
!  Output : filterfun : filter attenuation function value
!  ------
!---------------------------------------------------------------------
! modules
! =======
...
!
      DO p = 1, filt_ord
         exp = exp * kk * kk * filt_length * filt_length
         val = val + filt_coef(p) * exp
      ENDDO
!
      IF (val.LE.FREAL(0.0)) GOTO 1000
!
      filterfun = kk / val
!
!---------------------------------------------------------------------
...
! --- error management
!
 1000 CALL printerror2(0,1000,1,'utilfilt','filterfun')
!
      END FUNCTION
!%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
! -----------------------------------------------------------------
!%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
      SUBROUTINE dhankl(BMAX,NB,NREL,TOL,NTOL,NORD,FUN1,IJREL,DWORK, &
     &                  DANS,ARG,NOFUN1,IERR)
!---------------------------------------------------------------------
! 
!  Purpose : Compute Hankel transform
!  -------
!  Method : See description below
!  ------
!---------------------------------------------------------------------
      use mod_cont
      INTEGER, intent(in) :: NB,NREL,NTOL,NORD(NREL),IJREL(2,NREL)
      BIGREAL, intent(in) :: BMAX,TOL
      BIGREAL, intent(inout) :: DWORK(801,NREL)
      INTEGER, intent(out) :: NOFUN1,IERR
      BIGREAL, intent(out) :: DANS(NB,NREL),ARG(NB)
      INTERFACE
         FUNCTION fun1 (kk)
         use mod_main
         IMPLICIT NONE
      ...
      END INTERFACE
!=======================================================================
      BIGREAL :: ABSCIS,C,CMAX,DSUM,E,ER,G,Y,Y1
      BIGREAL, dimension(801) :: WT0, WT1
      INTEGER, dimension(801) :: KEY
      INTEGER :: I,NB1,LAG,ILAG,ISTORE,JREL,NONE,ITOL
      INTEGER :: J,M,LOOK,IQ,IR,IROLL
!=======================================================================
!
!  PURPOSE
!
!    THE PURPOSE OF SUBPROGRAM DHANKL IS TO PROVIDE IN DOUBLE PRECISION
!    A GENERAL ALGORITHM FOR FAST REAL HANKEL TRANSFORMS OF ORDERS
!    0 AND 1 USING RELATED AND LAGGED CONVOLUTIONS.
!
!  AUTHOR
!
...
!       FOURIER AND HANKEL TRANSFORM INTEGRALS. N.T.I.S REPT.
!       PB-242-800, SPRINGFIELD, VA., 1975.
!
!    2. ANDERSON, W.L., NUMERICAL INTEGRATION OF RELATED HANKEL
!       TRANSFORMS OF ORDERS 0 AND 1 BY ADAPTIVE DIGITAL FILTERING.
!       GEOPHYSICS 44 (JULY 1979), 1287-1305.
!
!  LANGUAGE
!
!    ANS-FORTRAN (X3.9-1966) IS USED, WITH THE EXCEPTION OF THE
!    CHARACTERS <,[,&,:,],> APPEARING IN SOME COMMENT STATEMENTS.
!
!  ABSTRACT
!
!      BY COMBINING BOTH ADAPTIVE LAGGED CONVOLUTION (SEE [1]) AND
!    ADAPTIVE RELATED CONVOLUTION (SEE [2]), SUBPROGRAM DHANKL
!    MINIMIZES EXTERNAL FUN1 CALLS (NOFUN1 AT EXIT) IN EVALUATING A
!    TOTAL OF NB*NREL REAL HANKEL TRANSFORMS OF ORDERS 0 AND (OR) 1,
!    WHERE NB IS THE NUMBER OF LAGGED CONVOLUTIONS, AND NREL IS THE
!    NUMBER OF RELATED CONVOLUTIONS.
!      DIRECT CONVOLUTION METHODS (SEE [1],[2]) DO NOT REQUIRE BESSEL
!    FUNCTION EVALUATIONS, AND HENCE ARE GENERALLY AN ORDER OF
!    MAGNITUDE FASTER TO COMPUTE THAN MOST DIRECT NUMERICAL
!    INTEGRATION METHODS.  BY USING PREVIOUSLY SAVED TRANSFORM INPUT
!    FUNCTION EVALUATIONS, BOTH LAGGED CONVOLUTION AND RELATED
!    CONVOLUTION FURTHER REDUCE SIGNIFICANTLY THE NUMBER OF TRANSFORM
!    INPUT FUNCTION EVALUATIONS REQUIRED OVER DIRECT CONVOLUTION.
!      LAGGED CONVOLUTION IS SELECTED WHEN NB>1, WHICH DEFINES ARG(NB)
!    OVER ANY DESIRED TRANSFORM ARGUMENT RANGE (BMIN,BMAX). RESULTS ARE
!    STORED AT THE FILTER SPACING IN ARRAYS ARG(NB) AND DANS(NB,NREL)
!    FOR LATER USE IN SPLINE INTERPOLATION, ETC.  GIVEN BMAX,NB, THE
!    VALUE OF BMIN (NOT GIVEN) CAN BE COMPUTED FROM THE EXPRESSION
!    BMIN=BMAX*DEXP(-.1D0*(NB-1)), WHICH MUST BE .GT. 0.0D0 FOR THE
!    GIVEN MACHINE EXPONENT RANGE.
!      RELATED CONVOLUTION IS SELECTED WHEN NREL>1, AND BY GIVEN
!    SIMPLE ALGEBRAIC RELATIONSHIPS BETWEEN FUN1 AND EACH NREL RELATED
!    TRANSFORM INPUT FUNCTION, DEFINED AS G**I * FUN1(G)**J, WHERE
!    FUN1(G) IS THE FIRST TRANSFORM INPUT FUNCTION, AND ARRAY
!    IJREL(2,NREL)= PAIRS OF I,J INTEGERS (NEGATIVE, 0, OR POSITIVE).
!    THE ORDER OF ALL RELATED CONVOLUTIONS MUST BE GIVEN IN NORD(NREL),
!    AND MUST BE EITHER 0 OR 1, BUT CAN BE IN ANY DESIRED SEQUENCE.
!    HIGHER INTEGER ORDERS MAY BE EXPRESSED IN TERMS OF ORDERS 0 AND 1
!    BY USING THE RECURRENCE RELATION JN-1(X)+JN+1(X)=2*N*JN(X)/X.
!      THE EQUALLY-SPACED J0,J1 FILTER ABSCISSAS ARE GENERATED IN
!    DOUBLE-PRECISION (TO CONSERVE STORAGE) AND ARE USED AS ARGUMENTS
!    IN THE EXTERNAL DOUBLE PRECISION FUNCTION FUN1.
!    BOTH J0 AND J1 FILTER RESPONSE FUNCTIONS (WEIGHTS) WERE
!    DESIGNED TO HAVE IDENTICAL ABSCISSA VALUES AS IN [2].  THE J0,J1
!    DOUBLE-PRECISION FILTER WEIGHTS USED IN SUBPROGRAM DHANKL WERE
!    REDESIGNED FROM [2] IN QUADRUPLE-PRECISION USING A VAX-11/780 IN
!    H-FLOATING ARITHMETIC.  MUCH OF THE LOGIC USED IN DHANKL FOLLOWS
!    THE CODING USED IN [1] AND [2].  THE MAJOR DIFFERENCES ARE IN THE
!    DEFINITION OF RELATED INPUT FUNCTIONS (SEE IJREL,DWORK), AND
!    FOR HANDLING OSCILLATING FUNCTIONS (SEE NTOL,ITOL).
!
!      FOUR GENERAL CASES ARE POSSIBLE USING SUBPROGRAM DHANKL.
!
!    CASE 1.  SINGLE DIRECT CONVOLUTION AT B=BMAX=BMIN (NB=1,NREL=1).
!    CASE 2.  RELATED CONVOLUTIONS AT A CONSTANT B=BMAX (NB=1,NREL>1).
!    CASE 3.  LAGGED CONVOLUTIONS IN (BMIN,BMAX) (NB>1,NREL=1).
!    CASE 4.  BOTH RELATED AND LAGGED CONVOLUTIONS (NB>1,NREL>1).
!
!  MACHINE DEPENDENT REMARKS
!
!      THIS SUBPROGRAM WAS IMPLEMENTED AND TESTED ON A 64-BIT DOUBLE-
!    WORD MACHINE WITH EXP-RANGE APPROXIMATELY 10**-38 TO 10**+38 AND
!    56-BIT MANTISSA (ABOUT 16-DECIMAL DIGITS).  ONLY DOUBLE-PRECISION
!    AND INTEGER OPERATIONS ARE USED.
!    FOR MACHINES WITH OTHER DOUBLE-WORD SIZES, CHANGES IN THE NUMBER OF
!    DIGITS RETAINED IN SOME DATA STATEMENTS MAY BE REQUIRED.
!
!  DESCRIPTION OF PARAMETERS
!
!    INPUT
!
!      BMAX - INITIAL HANKEL TRANSFORM ARGUMENT B=BMAX>0.D0 (ANY CASE),
!             USED IN INTEGRAL FROM 0 TO INFINITY OF
!             FUN1(G)*JN(G*B)*DG, WHERE JN=BESSEL FUNCTION OF ORDER N,
!             N=0 OR 1, AND B>0.0D0. (SEE FUN1 DEFINITION BELOW).
!        NB - NUMBER OF LAGGED CONVOLUTIONS DESIRED (NB.GE.1).  USE
!             NB=1 IF B=BMIN=BMAX (I.E., CASE 1 OR 2).  USE
...
!             A DIFFERENT B-SPACING THAN USED IN THE SAMPLED FILTERS. IF
!             SPLINE INTERPOLATION IS TO BE USED LATER, IT IS GENERALLY
!             BEST TO USE DLOG(ARG(I)) INSTEAD OF ARG(I) -VS- DANS(I,J),
!             FOR I=1,NB, AND FOR ANY GIVEN J BETWEEN 1 AND NREL.  NOTE
!             NB IS USED AS AN ADJUSTABLE DIMENSION IN DANS(NB,NREL).
!      NREL - NUMBER OF RELATED CONVOLUTIONS DESIRED (NREL.GE.1).  USE
!             NREL=1 IF ONLY A SINGLE HANKEL TRANSFORM IS USED.
!             NREL>1 REQUIRES ARRAY IJREL(2,NREL)  (SEE BELOW).
!             NOTE NREL IS USED AS ADJUSTABLE DIMENSIONS IN ARRAYS
!             DANS(NB,NREL),DWORK(801,NREL),NORD(NREL),IJREL(2,NREL).
!       TOL - REQUESTED TRUNCATION TOLERANCE AT BOTH FILTER TAILS
!             FOR ADAPTIVE CONVOLUTION FOR ALL NB*NREL TRANSFORMS.  THE
!             TRUNCATION CRITERION IS ESTABLISHED DURING CONVOLUTION IN
!             A FIXED ABSCISSA RANGE (USING WEIGHTS 299-339) OF EITHER
!             ORDER FILTER AS THE MAXIMUM ABSOLUTE CONVOLVED PRODUCT
...
!             NB*NREL REAL HANKEL TRANSFORMS IN DANS(NB,NREL).
!             HOWEVER, THE ACTUAL RELATIVE ERRORS CANNOT BE EXPECTED TO
!             BE SMALLER THAN ABOUT .1D-13 REGARDLESS OF THE TOLERANCE
!             VALUE USED, SINCE DOUBLE-PRECISION FILTER WEIGHTS AND
!             DOUBLE-PRECISION FUNCTIONS ARE USED. IN ANY EVENT,
!             ONE SHOULD ALWAYS CHOOSE TOL<<DESIRED RELATIVE ERROR.
!          ** ACCURACY WARNING **  SOME HIGHLY OSCILLATORY FUNCTIONS
!             FUN1(G) AND (OR) LIMITING CASES OF B NEAR MACHINE-ZERO
!             (OR INFINITY) SHOULD BE AVOIDED, OTHERWISE UNSATISFACTORY
!             RESULTS (E.G., RELATIVE & ABSOLUTE ERRORS>>TOL) MAY OCCUR.
!      NTOL - NUMBER OF CONSECUTIVE TIMES THE TRUNCATION CRITERION (TOL)
...
!      NORD - INTEGER ARRAY NORD(NREL) GIVING THE NREL ORDERS (0 OR 1)
!             OF EACH RELATED HANKEL TRANSFORM. IF ANY NORD(I),I=1,NREL,
!             IS NOT 0 OR 1, THEN ORDER 1 WILL BE ASSUMED.
!      FUN1 - NAME OF AN EXTERNAL DOUBLE PRECISION FUNCTION OF A REAL
!             ARGUMENT DEFINING THE 1ST TRANSFORM INPUT FUNCTION OF THE
!             SET OF RELATED TRANSFORMS TO BE EVALUATED.  AN EXTERNAL
!             FUN1 STATEMENT MUST APPEAR IN THE CALLING PROGRAM.  THE
!             DOUBLE PRECISION FUNCTION FUN1(G) MUST BE CODED BY THE
!             USER AND MUST BE A CONTINUOUS DECREASING REAL FUNCTION
!             FOR ALL REAL G>0.0D0.  THE VALUE OF G MUST BE UNCHANGED
!             UPON RETURN FROM FUN1. A MULTIPLE-POLE OF FUN1(G) AT G=0.0
!             CAN EXIST, PROVIDED THE HANKEL TRANSFORM CONVERGES (NOTE
!             FUN1(0D0) IS NOT USED).  GENERALLY, FUN1(G)
!             IS DEFINED ANALYTICALLY FOR ALL G>0.D0.  HOWEVER,
!             DISCRETELY DEFINED FUNCTIONS MAY BE USED IF FUN1(G)
!             RETURNS A SMOOTH INTERPOLATION VALUE (E.G., VIA CUBIC
!             SPLINES) WHICH SATISFIES THE CONTINUITY CONDITION FOR ALL
!             G>0, AND PROVIDED THE PROPER LIMITING VALUE OF FUN1(G) IS
!             GIVEN AS G TENDS TO INFINITY.  PARAMETERS OTHER
!             THAN G NEEDED IN FUN1(G) MAY BE INCLUDED BY USING LABELED
!             COMMON IN FUN1 AND IN THE USERS CALLING PROGRAM.  IF
!             FUN1(G) IS AN OSCILLATING FUNCTION, THEN THE HIGHEST
!             FREQUENCY COMPONENT (IN LOG-SPACE) SHOULD NOT EXCEED THE
!             FILTER NYQUIST FREQUENCY, 1/(2*0.1D0).  IN GENERAL,
!             SUBPROGRAM DHANKL PERFORMS BEST WHEN USING SMOOTH, WELL-
!             BEHAVED FUNCTIONS FUN1(G), THAT ARE CHARACTERIZED AS
!             MONOTONICALLY DECREASING FUNCTIONS WITH RELATIVELY FEW
!             ZEROS FOR G>0. (SEE THE ACCURACY WARNING UNDER TOL, AND
!             ERROR CONDITION (4).)
!     IJREL - INTEGER ARRAY IJREL(2,NREL) USED WHEN NREL>1 TO DEFINE
!             THE PAIR OF I,J INTEGER EXPONENTS FOR EACH RELATED INPUT
!             FUNCTION.  THE RELATED INPUT FUNCTIONS ARE ASSUMED
!             TO BE SIMPLY RELATED IN TERMS OF FUN1 VIA THE INTEGER
!             ARRAY IJREL(2,K),K=2,NREL.  THAT IS, WE ASSUME THE K-TH
!             RELATED INPUT FUNCTION IS GIVEN (SEE STATEMENT 160) AS
!             FUNK(G)=G**IJREL(1,K) * FUN1(G)**IJREL(2,K), WHERE
!             THE INTEGER EXPONENTS MAY BE POSITIVE, ZERO, OR NEGATIVE.
!             IN THIS WAY, ONLY FUN1 NEED BE DECLARED AN EXTERNAL
!             FUNCTION. MORE COMPLICATED CODE COULD BE USED
!             FOR THE RELATED FUNCTIONS, PROVIDED THE MEANING OF IJREL
!             IS REDEFINED AND STATEMENT 160 IS CHANGED (ALSO, SEE
!             ERROR CONDITION (3) BELOW). WHEN NREL=1, ARRAY
!             IJREL IS A DUMMY NAME (I.E., NOT REFERENCED).
!             IF NREL>1, THEN THE STATEMENT AT LABEL 160
!             IS DEFINED ONLY FOR K=J=2,...,NREL. THAT IS,
!             IJREL(1,1),IJREL(2,1) ARE NOT USED IN THIS VERSION.
!     DWORK - WORK ARRAY DWORK(801,NREL), WHICH IS USED TO
!             HOLD VARIOUS COMPUTED FUNCTIONAL VALUES DURING RELATED AND
!             LAGGED CONVOLUTIONS.  A STORAGE ROLL FEATURE USING
!             DWORK(801,NREL) AND INTERNAL ARRAY KEY(801) ALLOWS FOR
!             ANY B RANGE (BMIN,BMAX) TO BE USED DURING CONVOLUTION.
!
!  OUTPUT
!
!      DANS - THE ARRAY DANS(NB,NREL) IS RETURNED GIVING THE
!             NB*NREL REAL HANKEL TRANSFORMS, WITH CORRESPONDING
!             B ARGUMENTS GIVEN IN ARRAY ARG(NB).
!       ARG - THE ARRAY ARG(NB) IS RETURNED GIVING THE RESULTING
!             B ARGUMENTS IN (BMIN,BMAX), WHERE ARG(I+1)/ARG(I)=EXP(.1),
!             I=1,NB-1 (THIS ARRAY COULD BE ELIMINATED TO SAVE STORAGE
!             AND REGENERATED AFTER THE CALL DHANKL, IF DESIRED).
!    NOFUN1 - NUMBER OF DIRECT FUN1 EVALUATIONS USED FOR ALL NB*NREL
!             REAL HANKEL TRANSFORMS.  NOFUN1 IS USUALLY NOT MORE
!             THAN THE NUMBER OF WEIGHTS NEEDED FOR A SINGLE DIRECT
!             CONVOLUTION, FOR ANY NB AND NREL.
!      IERR - ERROR RETURN CODE.  THE FOLLOWING CODES ARE POSSIBLE --
!             = 0, NO ERROR IN INPUT PARAMETERS. DANS,ARG COMPUTED.
!             = 1, IMPROPER INPUT PARAMETERS (I.E., NB<1,NREL<1,BMAX<=0,
!             OR BMAX*DEXP(-.1D0*(NB-1))<=0.D0).  DANS,ARG NOT COMPUTED.
!
!
!  ERROR CONDITIONS
!
!    (1)  IMPROPER INPUT PARAMETERS GIVEN (SEE IERR=1 ABOVE).
!    (2)  UNDERFLOW CONDITIONS ARE POSSIBLE DURING CONVOLUTION, DUE TO
!         THE BEHAVIOR OF FUN1, VALUE OF B IN (BMIN,BMAX), TOL, AND
!         NTOL. EXPONENT AND (OR) ARITHMETIC UNDERFLOW TRAPS MUST RETURN
!         A VALUE OF 0.D0 FOR THE COMPUTER SYSTEM BEING USED.  NOTE THAT
!         UNDERFLOW MAY ALSO OCCUR IN THE USERS EXTERNAL FUNCTION
!         FUN1(G) FOR ANY VALUE OF G AS SET BY SUBPROGRAM DHANKL.
!    (3)  AN UNRECOVERABLE OVERFLOW CONDITION CAN OCCUR IN EXECUTING
!         STATEMENT 160, DEPENDING ON THE VALUE OF B IN (BMIN,BMAX),
!         TOL, OR THE INTEGER EXPONENTS USED IN IJREL(2,NREL),NREL>1.
!         IN GENERAL, EXTREMELY SMALL OR LARGE VALUES OF B SHOULD BE
!         AVOIDED (SEE ACCURACY WARNING UNDER TOL ABOVE).  ALSO, IN MANY
!         CASES, EXPONENT OVERFLOW CAN BE AVOIDED BY PROPER CHOICE OF
!         FUN1 AND THE RELATED INPUT FUNCTION ORDERING DEFINED BY
!         THE IJREL SIGNED INTEGER EXPONENTS.
!    (4)  UNDETECTED ERRORS ARE POSSIBLE IF FUN1 IS IMPROPERLY CODED, OR
!         DOES NOT YIELD DOUBLE-PRECISION ACCURACY, OR IS NOT
!         A CONTINUOUS DECREASING REAL FUNCTION FOR ALL G>0.D0.
!
!  USAGE
!
!    SUBPROGRAM DHANKL IS CALLED AS FOLLOWS (USE NUMERICAL VALUES FOR
!    <EXPRESSION>, EXCLUDING < AND >, IN DECLARATIONS) --
!
!      DOUBLE PRECISION BMAX,TOL,DWORK(801,<NREL>),DANS(<NB>,<NREL>),
!     1 ARG(<NB>)
!      DIMENSION NORD(<NREL>),IJREL(2,<NREL>)
!      EXTERNAL DFUN1
!-----READ/LOAD INPUT PARAMETERS FOR DHANKL AS REQUIRED
!      ...
!      CALL DHANKL(BMAX,NB,NREL,TOL,NTOL,NORD,DFUN1,IJREL,DWORK,
!     * DANS,ARG,NOFUN1,IERR)
!      IF(IERR.EQ.1) STOP
!      ...
!      END
!      DOUBLE PRECISION FUNCTION DFUN1(G)
!      DOUBLE PRECISION G
!-----INSERT USER SUPPLIED CODE FOR EVALUATION OF DFUN1(G),G>0.0D0
!      END
!
!=======================================================================
!-----WE DEFINE C,CMAX FOR USE IN THE TRUNCATION CRITERION TESTS,
!     WHERE C IS ANY CONVOLUTION PRODUCT AND CMAX IS THE MAXIMUM
!     CONVOLVED PRODUCT IN THE FIXED ABSCISSA RANGE (SEE PARAMETER TOL).
!-----ABSCIS=BASE CONSTANT FOR FILTER ABSCISSA GENERATION
      DATA ABSCIS/0.7059431685223780D0/
!-----E=DEXP(.1D0), ER=1.0D0/E (ALSO USED IN ABSCISSA GENERATION)
      DATA E/1.10517091807564762D0/,ER/.904837418035959573D0/
!-----WT0(I)=J0 HANKEL TRANSFORM FILTER WEIGHTS FOR I=1,801
      DATA &
...
!-----WT1(I)=J1 HANKEL TRANSFORM FILTER WEIGHTS FOR I=1,801
      DATA &
...
!-----ERROR CHECKS
      ...
      IERR=0
!-----INITIALIZE RELATED CONVOLUTION WITHIN LAGGED CONVOLUTION LOOPS
      DO 10 I=1,801
   10   KEY(I)=0
      NB1=NB+1
      LAG=-1
!-----PRESET INITIAL FILTER ABSCISSA FOR STARTING BMAX, THE ARGUMENT
!     USED IN THE EXTERNAL FUNCTION FUN1(G).  NOTE THE ABSCISSAS
!     ARE EQUALLY SPACED (E=DEXP(.1D0), ER=1.0D0/E) IN LOG-SPACE.
      Y1=ABSCIS/BMAX
!-----LAGGED CONVOLUTION, OUTERMOST LOOP 1010
      DO 1010 ILAG=1,NB
        LAG=LAG+1
        ISTORE=NB1-ILAG
        IF(LAG.GT.0) Y1=Y1*E
        ARG(ISTORE)=ABSCIS/Y1
!-------RELATED CONVOLUTION, INNERMOST LOOP 1000
        DO 1000 JREL=1,NREL
!---------SPECIAL CASE FLAG NONE=1 IS SET IF FUN1(G)=0 FOR ALL G IN
!         FILTER FIXED RANGE (USING WEIGHTS 229-339).
          NONE=0
          ITOL=NTOL
          DSUM=0.0D0
          CMAX=0.0D0
          Y=Y1
!---------BEGIN RIGHT SIDE CONVOLUTION AT WEIGHT 299 (M=RETURN LABEL)
          ASSIGN 20 TO M
          I=299
          Y=Y*E
!---------CALL PSEUDO SUBROUTINE AT 100 (RETURN TO 20 VIA M ASSIGNED)
          GO TO 100
...
!---------ESTABLISH TRUNCATION CRITERION
          CMAX=TOL*CMAX
          ASSIGN 30 TO M
!---------CALL PSEUDO SUBROUTINE AT 100 (RETURN TO 30 VIA M ASSIGNED)
          GO TO 100
...
!---------CALL PSEUDO SUBROUTINE AT 100 (RETURN TO 30 VIA M ASSIGNED)
   ...
   50     ITOL=ITOL-1
          ...
          ITOL=NTOL
          Y=Y1
!---------CONTINUE WITH LEFT SIDE CONVOLUTION AT WEIGHT 298
          ASSIGN 60 TO M
          I=298
!---------CALL PSEUDO SUBROUTINE AT 100 (RETURN TO 60 VIA M ASSIGNED)
          GO TO 100
...
!---------NORMALIZE DSUM BY ARG(ISTORE) TO ACCOUNT FOR INTEGRATION
!         RANGE CHANGE, AND STORE IN DANS(ISTORE,JREL)
          DANS(ISTORE,JREL)=DSUM/ARG(ISTORE)
!---------SKIP OVER PSEUDO SUBROUTINE TO END OF DO 1000 INNERMOST LOOP
          GO TO 1000
!
!=======================================================================
!=====STORE/RETRIEVE PSEUDO SUBROUTINE FOR RELATED/LAGGED CONVOLUTION.
!     THE INTERNAL (PSEUDO) SUBROUTINE ENTRY IS LABEL 100, AND RETURNS
!     TO THE LABEL ASSIGNED TO M.  THIS CALLING MECHANISM COULD OCCUR
!     A MAXIMUM OF 801*NB*NREL TIMES, WHERE PARAMETERS NB>0 AND NREL>0
!     CAN BE ARBITRARILY LARGE.  IF A MORE-STRUCTURED STANDARD FORTRAN
!     SUBROUTINE CALL WAS USED, THEN THE USUAL COMPILER LINKAGE
!     CONVENTION COULD GENERATE A MAXIMUM OF 801*NB*NREL MACHINE-
!     LANGUAGE INSTRUCTIONS FOR REGISTER SAVES/RESTORES AND OTHER
!     MEMORY REFERENCES.  FOR MOST COMPILERS, TIMING TESTS REVEAL THAT
!     THE PSEUDO-CALL METHOD USED HERE GENERATED FASTER MACHINE CODE
!     THAN WITH USING EXTERNAL SUBROUTINE CALLS (E.G., CALL LINKAGE
!     VERSUS PSEUDO-CALL RATIO WAS 2.6:1 ON A VAX-11/780 USING
!     NB=50,NREL=61, AND NOFUN1=199).
!
!=========PSEUDO-CALL ENTRY POINT AT 100 (RETURNS VIA GO TO M BELOW)
  100     LOOK=I+LAG
          IQ=LOOK/802
          IR=MOD(LOOK,802)
          IF(IR.EQ.0) IR=1
...
!=========USE EXISTING SAVED FUNCTIONAL VALUES IN DWORK(IR,JREL)
  110     IF(NORD(JREL)) 130,120,130
  120     C=DWORK(IR,JREL)*WT0(I)
          GO TO 140
  130     C=DWORK(IR,JREL)*WT1(I)
  140     DSUM=DSUM+C
!=========RETURN CONVOLUTION CONTROL VIA ASSIGNED M VALUE, AND WITH
!         THE LAST CONVOLUTION PRODUCT (C)
          GO TO M,(20,30,60)
!=========COMPUTE EXTERNAL FUN1 DIRECTLY ONLY WHEN NECESSARY
  150     KEY(IR)=IROLL+IR
          G=Y
          DWORK(IR,1)=FUN1(G)
          NOFUN1=NOFUN1+1
          IF(NREL.EQ.1) GO TO 110
!=========FILL-IN REMAINING RELATED DWORK(IR,J),J=2,NREL FOR THIS IR
          DO 160 J=2,NREL
!***********FOR OTHER THAN SIMPLE RELATIONS, THE FOLLOWING STATEMENT
!           COULD BE CHANGED (AND ALSO THE MEANING OF IJREL(2,NREL)).
  160    DWORK(IR,J)=(G**IJREL(1,J))*DWORK(IR,1)**IJREL(2,J)
          GO TO 110
!=========END OF PSEUDO SUBROUTINE (ENTRY 100, RETURN GO TO M ABOVE)
!=======================================================================
!
!-------END LOOP 1000 (GET REMAINING RELATED CONVOLUTIONS FOR THIS ARG)
 1000   CONTINUE
!-----END LOOP 1010 (GET REMAINING LAGGED CONVOLUTIONS FOR NEXT ARG)
 1010 CONTINUE
!-----EXIT WITH DANS(NB,NREL),ARG(NB) COMPLETED WITH MINIMAL FUN1 CALLS
      RETURN
 9999 IERR=1
      RETURN
      END SUBROUTINE
